...
def main():
    ...
    ap.add_argument("--zero-shift", type=float, default=0.0,
                    help="Constant 2theta zero-point offset (deg) added to all peaks")
    ap.add_argument("--displacement", type=float, default=0.0,
                    help="Sample-displacement scale (deg): adds D*cos(theta) to 2theta")
    ...
    if args.zero_shift or args.displacement:
        _theta_rad = np.radians(two_theta / 2.0)
        two_theta = two_theta + args.zero_shift + args.displacement * np.cos(_theta_rad)
        print(f"[robust] zero_shift={args.zero_shift} deg  displacement={args.displacement} deg")
