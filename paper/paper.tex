...
\makeatother
\hypersetup{
  pdftitle={PXRD-Diff: diagnosing the encoder bottleneck in diffusion-based powder-diffraction structure inversion{,} with classical autoindexing as a drop-in remedy},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{PXRD-Diff: diagnosing the encoder bottleneck in diffusion-based
powder-diffraction structure inversion, with classical autoindexing as a
drop-in remedy}
\author{}
\date{}
...
\textbf{Synopsis:} A conditional diffusion model for powder-diffraction
structure inversion is diagnosed: global pooling in the pattern encoder
destroys the absolute d-spacing information that lattice recovery
requires, and substituting a classical Q-space autoindexer for the
learned lattice head yields a statistically significant improvement.
Three external generative baselines are re-scored on a single shared
evaluation harness.
...
three-dimensional crystal structure remains an open problem, and recent
generative approaches report results under mutually incompatible
protocols with failure modes seldom catalogued. PXRD-Diff, a 3.7
M-parameter conditional diffusion model trained on the CDVAE MP-20
split, is presented, and DiffractGPT, PXRDnet and deCIFer are reproduced
through the same pymatgen StructureMatcher harness. The central,
statistically robust finding is mechanistic: the learned PXRD encoder
reaches a 0.007 normalised auxiliary lattice-regression loss in
isolation, yet substituting that head's own lattice into the sampler
recovers only 1.2 \% of structures, because its \textasciitilde1.1 Å
length error shifts every Bragg peak by \textasciitilde15 \%. The
limiter is the global pooling that discards absolute peak positions, not
the denoiser alone. A perturbation study locates a sharp sensitivity
knee at \textasciitilde0.5 Å cell error, and a true-lattice oracle lifts
match to 4.5 \% at n = 1000 (three seeds; 95 \% Wilson confidence
interval {[}3.8, 5.3{]}), disjoint from the no-true-lattice indexer's
1.5 \% {[}1.2, 2.0{]}. Replacing the learned lattice head with a
classical Q-space autoindexer raises no-true-lattice match to 1.5 \%
from a learned head that recovers essentially nothing; a three-seed
paired McNemar test confirms the gain is significant (p \textless{}
10⁻⁴; 27 fixed, none broken), concentrated in the high-symmetry systems
below the sensitivity knee. Reproduced on one harness, deCIFer reaches
73.8 \% match, DiffractGPT 18.9 \% and PXRDnet 30.0 \%, leaving
PXRD-Diff 12--46 times behind. The contributions are the
encoder-bottleneck diagnosis, the classical-indexer drop-in, a four-item
failure catalogue and the shared evaluation harness; code, checkpoints
and the differentiable Bragg simulator are released.
...
\item
  ...
  landscape is too rough for gradient descent (Segal \emph{et al.},
  2025).
\item
...
de-Wolff dichotomy method, ITO, TREOR, and DICVOL (Boultif \& Louër,
2004) index high-symmetry powder patterns in CPU milliseconds, with the
difficulty concentrated in low-symmetry (monoclinic/triclinic) cells.
§3.5 reuses this machinery as a drop-in for the learned lattice head,
and §5.2/§5.5 show the resulting lift tracks exactly the
...
spectral targets for amorphous and nanostructured systems (Guo \&
Schwalbe-Koda, 2026). Relevant to our negative results, Segal \emph{et
al.} (2025) show the powder-XRD similarity loss landscape is ``too rough
for gradient descent'', independent evidence for why our sampling-time
Debye-gradient guidance fails (§5.4).
...
to steer the trajectory, consistent with Segal \emph{et al.} (2025), who
...
R. (2025). The loss landscape of powder X-ray diffraction-based
