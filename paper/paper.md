...
2. **Encoder-bottleneck diagnosis (our primary contribution).** A perturbation study (§5.5) locates a sharp ~0.5 Å sensitivity knee; a true-lattice oracle lifts match from 1.6 % to **5.6 %** at n = 1000, with non-overlapping 95 % Wilson CIs. The encoder reaches a 0.007 *normalised* aux lattice loss in isolation, but substituting that head's lattice into the sampler recovers only 1.2 % (§5.6) — its ~1.1 Å error sits well above the knee — so the pooled encoding, not merely the denoiser, fails to deliver absolute d-spacings. The lattice channel is the dominant *accessible* limiter; the 5.6 % oracle ceiling shows coordinates remain a co-bottleneck.
...
A physics-informed auxiliary loss and a released, validated differentiable tool. (We make no claim that this loss by itself improves match rate; see §5.1.) We implement, in PyTorch,
$$
...
Two choices are load-bearing. The **lattice-input fix** (§3.4) drops lattice loss from 1.0 (pinned at the prior variance for 100 k steps) to ~0.02 once $W_\text{lat}\boldsymbol{\ell}^{(t)}$ enters the head input. Switching from **ε to x₀-residual** at fixed Debye λ = 1 (v11 → v13) lifts match 0.9 % → 2.5 % — 3.5× over the v10 ε baseline. The differentiable Debye term, by contrast, is *not* load-bearing: under ε-prediction, adding it lowered match (v10 1.40 % → v11 0.90 %, Table 1), and the early λ_Debye sweep {0, 0.1, 1, 10} showed no measurable effect (Appendix B). We did not isolate the Debye term under the winning x₀-residual configuration (a v13 run at λ_Debye = 0 is the missing ablation), so we make no claim that it improves match rate; we retain it at λ = 1 and release the validated simulator (§3.3) as a reusable tool rather than a demonstrated performance gain. Capacity is not the bottleneck: a 10.1 M-parameter run ($d=384$, $L=5$, `gpu_v12`) stayed at the same ~1.0 coord-loss plateau as v10. Under the full pipeline (sampled lattice), v13 drops to 1.2 % at n = 256; the Phase 9 v21 checkpoint reaches 5.6 % under oracle but only 1.0 % under full pipeline. §5.2 attacks that gap.
...
DiffractGPT (`knc6/diffractgpt_mistral_chemical_formula`) at n = 1 000 (990 scored after 10 CIF-parse failures) and PXRDnet (`therealgabeguo/cdvae_xrd_sinc100`) at n = 20 (limited by ~4 GPU-hr/material) re-scored on *our* `StructureMatcher` harness at matching tolerances. Crystalyze's checkpoint download is inactive; cite-but-not-reproduced. Note that DiffractGPT emits its own formula and so does not have composition handed to it: its composition match is 83.5 %, so its all-correct denominator is the 990 scored generations rather than a full known-composition set — a mild asymmetry that makes its reported rates conservative relative to PXRD-Diff (which is given Z).
...
| DiffractGPT [^DiffractGPT]                   | 990   | 18.9 [16.6, 21.4]  | 15.9 [13.8, 18.3]       | 21.3     | 0.001        |
...
Perturbing the oracle (true) lattice with Gaussian cell-length noise (σ_ang = 0), v21 at n = 300: match collapses from 2.0 % at zero perturbation to 0.33 % once the realised cell-length MAE reaches 0.41 Å (σ_len = 0.5 Å), stays at 0.33 % through 0.81 Å, and reaches 0 % for MAE ≥ 1.0 Å — an even sharper drop than a linear knee, with essentially all match lost beyond ~0.4 Å of cell-length error. (This n = 300 sweep's unperturbed baseline of 2.0 % sits below the n = 1000 oracle's 5.6 % in §5.2; they are different test subsets, so we read the sweep as a *relative* decay from its own baseline rather than anchoring it to the oracle — reconciling the two baselines on a common subset is a loose end.) The direction is consistent with the indexer result: §5.2's lift comes from the high-symmetry systems, whose *symmetry-consistent* cell errors degrade match less than the unstructured Gaussian perturbation here — which breaks parameter couplings such as a = b in hexagonal — so we treat the perturbation knee and the indexer's per-system MAE as qualitatively consistent (cell-length error is the dominant accessible limiter) rather than as numerically identical thresholds. §5.2's indexer lift is *predictable* — it works where it can.
...
- **Statistics.** The Phase 4 ablations are single-seed; their large effects (the 2.5–3.5× x₀-residual and lattice-input lifts) are robust to this, and the oracle-vs-indexer gap has disjoint CIs. The indexer-vs-learned-head comparison was rerun at three seeds and the lift confirmed significant by a paired McNemar test (p < 10⁻⁴; §5.2) — superseding the earlier underpowered unpaired test (p = 0.24). We did not multi-seed the oracle or the Phase 4 ablation runs.
