...
**The lift is real: a three-seed paired test.** The single-seed snapshot alone does not establish a genuine improvement — its Wilson CIs overlap and an unpaired two-proportion test gives p = 0.24. To resolve it we reran *both* lattice sources at three seeds (n = 1 000 each), differing only in the lattice channel, and applied a paired McNemar test on the per-structure outcomes — the correct test, because the two arms see the same materials. The result is unambiguous. Pooled over seeds the learned-head full pipeline recovers **0 / 1 949** evaluable structures (0.0 % [0.0, 0.2]), while the indexer drop-in recovers **1.5 % [1.2, 2.0]** (46 / 3 000), reproducing the single-seed 1.6 %. The paired improvement is **significant — McNemar p < 10⁻⁴**, with 27 structures recovered by the indexer that the learned head missed and *zero* broken in the other direction. (The single-seed 1.0 % learned-head figure in Table 2 is within run-to-run noise of zero; across three fresh seeds the learned lattice head matched nothing.) The gain does **not** carry to the stricter all-correct metric, where the paired test is not significant (4 vs 0 discordant, p = 0.13) — the indexer fixes the cell but not the space group. So the drop-in reproducibly improves match rate over the learned lattice head; the *mechanism* (§5.5) is that its gains come entirely from high-symmetry systems where the indexer's cell error falls below the 0.5 Å sensitivity knee, with low-symmetry contributing nothing.
